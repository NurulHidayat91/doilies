		...
		<div class="box box-warning box-solid">
			<div class="box-header with-border">
				<?php if ($button === "Update") { ?>
					<h3 class="box-title">UPDATE DATA SEALER</h3>
				<?php } elseif ($button === "Repeat") { ?>
					<h3 class="box-title">REPEAT DATA SEALER</h3>
				<?php } else { ?>
					<h3 class="box-title">INPUT DATA SEALER</h3>
				<?php } ?>
			</div>
			<form action="<?php echo $action; ?>" method="post">
				...
				<table class='table table-bordered>'>
					...
					<tr>
						...
						<?php if ($button === 'Update') { ?>
							<td>
								<input type="date" class="form-control" readonly name="tanggal" id="tanggal" placeholder="Tanggal" value="<?php echo $tanggal; ?>" />
							</td>
						<?php } else { ?>
							<td>
					...
					<tr>
						...
						<td>
							...
							<!-- <input type="text" readonly class="form-control" name="id_customer" id="id_customer" placeholder="Nama Customer" value="<?php echo $id_customer; ?>" /> -->
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td></td>
						<td><input type="hidden" name="id_mesin_sealer" value="<?php echo $id_mesin_sealer; ?>" />
							<button type="submit" class="btn btn-danger"><i class="fa fa-floppy-o"></i> <?php echo $button ?></button>
							<a href="<?php echo site_url('mesinsealer') ?>" class="btn btn-info"><i class="fa fa-sign-out"></i> Kembali</a>
						</td>
					</tr>
				</table>
			</form>
...
<script type="text/javascript">
	function autoFill() {
		...
		$.ajax({
			url: "<?= base_url() ?>mesinsealer/autofill",
			type: "GET",
			data: {
				'no_wo': no_wo
			},
