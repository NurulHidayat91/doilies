...
{
    $ci = get_instance();
    $string = '<input value="' . $value . '" name="' . $name . '" list="' . $name . '" class="form-control" placeholder="Ketik disini......">
    <datalist id="' . $name . '">';
    $data = $ci->db->get($table)->result();
    foreach ($data as $row) {
...
{
    $CI         = &get_instance();
    $tipeuser   = $CI->session->id_user_level;
...
function getFieldValue($table, $field, $key, $value)
{
    ...
    return !isset($data[$field]) ? null : $data[$field];
}

function check_admin()
{
    $ci     = &get_instance();
    if ($ci->session->userdata('id_user_level') == 3) {
        redirect('dashboard');
    }
}
