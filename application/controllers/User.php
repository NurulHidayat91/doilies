...
class User extends CI_Controller
{
    ...
    public function update($id)
    {
        ...
        if ($row) {
            $data = array(
                ...
                'id_jabatan'    => set_value('jabatan', $row->id_jabatan),
                'id_divisi'     => set_value('divisi', $row->id_divisi),
    ...
    public function update_profile($id)
    {
        ...
        if ($row) {
            $data = array(
                'button'        => 'Update',
                'action'        => site_url('user/update_action_profile'),
                'id_users'      => set_value('id_users', $row->id_users),
                'full_name'     => set_value('full_name', $row->full_name),
                'id_jabatan'     => set_value('jabatan', $row->id_jabatan),
                'id_divisi'     => set_value('divisi', $row->id_divisi),
                ...
                'is_aktif'      => set_value('is_aktif', $row->is_aktif),
                'row'           => $this->User_model->get_by_id($id),
                'level'    => $this->User_level_model->get_all($id)
            );
            ...
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('user/update_profile/' . $this->session->id_users));
        }
    }

    public function update_action_profile()
    {
        $this->_rules();
        $foto = $this->upload_foto();
        if ($this->form_validation->run() == FALSE) {
    ...
    public function _rules()
    {
        ...
        $this->form_validation->set_rules('id_user_level', 'id user level', 'trim');
        $this->form_validation->set_rules('is_aktif', 'is aktif', 'trim');
