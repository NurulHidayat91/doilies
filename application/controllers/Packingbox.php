...
class Packingbox extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Packingbox_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            ...
            $config['first_url'] = base_url() . 'index.php/packingbox/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'index.php/packingbox/index/';
            $config['first_url'] = base_url() . 'index.php/packingbox/index/';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = FALSE;
        $config['total_rows'] = $this->Packingbox_model->total_rows($q);
        $packingbox = $this->Packingbox_model->get_limit_data($config['per_page'], $start, $q);
        $packingbox_peruser = $this->Packingbox_model->get_limit_peruser($config['per_page'], $start, $q);
        ...
        $data = array(
            'packingbox_data' => $packingbox,
            'packingbox_data_peruser' => $packingbox_peruser,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->template->load('template', 'packingbox/packingbox_list', $data);
    }

    public function read($id)
    {
        $row = $this->Packingbox_model->get_by_id($id);
        if ($row) {
            $data = array(
                'id_packingbox' => $row->id_packingbox,
                'no_wo' => $row->no_wo,
                'id_user' => $row->id_user,
                'tanggal' => $row->tanggal,
            ...
            $this->template->load('template', 'packingbox/packingbox_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('packingbox'));
        }
    }
    ...
    public function create()
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('packingbox/create_action'),
            'id_packingbox' => set_value('id_packingbox'),
        ...
        $this->template->load('template', 'packingbox/packingbox_form', $data);
    }
    ...
    public function create_manual()
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('packingbox/create_action_manual'),
            'id_packingbox' => set_value('id_packingbox'),
            'no_wo' => set_value('no_wo'),
            'id_user' => set_value('id_user'),
            'tanggal' => set_value('tanggal'),
        ...
        $this->template->load('template', 'packingbox/packingbox_form_manual', $data);
    }
    ...
    public function create_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // var_dump($data);
            // die;
            $this->Packingbox_model->insert($data);
    ...
    public function update($id)
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('packingbox/update_action'),
                'id_packingbox' => set_value('id_packingbox', $row->id_packingbox),
    ...
    public function update_action()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $this->update($this->input->post('id_packingbox', TRUE));
        } else {
            $data = array(
                'no_wo' => $this->input->post('no_wo', TRUE),
                'id_user' => $this->session->userdata('id_users'),
            ...
            $this->Packingbox_model->update($this->input->post('id_packingbox', TRUE), $data);
    ...
    public function delete($id)
    {
        ...
        if ($row) {
            $this->Packingbox_model->delete($id);
    ...
    public function _rules()
    {
        ...
        $this->form_validation->set_rules('id_packingbox', 'id_packingbox', 'trim');
    ...
    public function excel()
    {
        ...
        foreach ($this->Packingbox_model->get_all() as $data) {
            $kolombody = 0;
...
/* End of file Packingbox.php */
/* Location: ./application/controllers/Packingbox.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2023-08-18 03:52:01 */
/* http://harviacode.com */