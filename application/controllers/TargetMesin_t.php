...
class TargetMesin_t extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		is_login();
		$this->load->model('TargetMesin_t_model');
	...
	public function index()
	{
		...
		if ($q <> '') {
			...
			$config['first_url'] = base_url() . 'index.php/targetmesin_t/index.html?q=' . urlencode($q);
		} else {
			$config['base_url'] = base_url() . 'index.php/targetmesin_t/index/';
			$config['first_url'] = base_url() . 'index.php/targetmesin_t/index/';
		}

		$config['per_page'] = 10;
		$config['page_query_string'] = FALSE;
		$config['total_rows'] = $this->TargetMesin_t_model->total_rows($q);
		$targetmesin_t = $this->TargetMesin_t_model->get_limit_data($config['per_page'], $start, $q);
		...
		$data = array(
			'targetmesin_t_data' => $targetmesin_t,
			'q' => $q,
			'pagination' => $this->pagination->create_links(),
			'total_rows' => $config['total_rows'],
			'start' => $start,
		);
		$this->template->load('template', 'targetmesin_t/tbl_target_mesin_t_list', $data);
	}

	public function read($id)
	{
		$row = $this->TargetMesin_t_model->get_by_id($id);
		if ($row) {
			$data = array(
				'id_targetMesin_t' => $row->id_targetMesin_t,
			...
			$this->template->load('template', 'targetmesin_t/tbl_target_mesin_t_read', $data);
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
			redirect(site_url('targetmesin_t'));
		}
	}
	...
	public function create()
	{
		$data = array(
			'button' => 'Create',
			'action' => site_url('targetmesin_t/create_action'),
			'id_targetMesin_t' => set_value('id_targetMesin_t'),
		...
		$this->template->load('template', 'targetmesin_t/tbl_target_mesin_t_form', $data);
	}
	...
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->TargetMesin_t_model->insert($data);
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('targetmesin_t/update_action'),
				'id_targetMesin_t' => set_value('id_targetMesin_t', $row->id_targetMesin_t),
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			$this->update($this->input->post('id_targetMesin_t', TRUE));
		} else {
			$data = array(
				'id_bentuk' => $this->input->post('id_bentuk', TRUE),
				'id_grade' => $this->input->post('id_grade', TRUE),
			...
			$this->TargetMesin_t_model->update($this->input->post('id_targetMesin_t', TRUE), $data);
	...
	public function delete($id)
	{
		...
		if ($row) {
			$this->TargetMesin_t_model->delete($id);
	...
	public function _rules()
	{
		...
		$this->form_validation->set_rules('id_targetMesin_t', 'id_targetMesin_t', 'trim');
	...
	public function excel()
	{
		...
		$namaFile = "tbl_target_mesin_t.xls";
		$judul = "tbl_target_mesin_t";
		$tablehead = 0;
		$tablebody = 1;
		$nourut = 1;
		//penulisan header
		...
		foreach ($this->TargetMesin_t_model->get_all() as $data) {
			$kolombody = 0;
...
/* End of file TargetMesin_t.php */
/* Location: ./application/controllers/TargetMesin_t.php */
