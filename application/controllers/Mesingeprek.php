...
class Mesingeprek extends CI_Controller
{
	function __construct()
	{
		parent::__construct();
		is_login();
		$this->load->model('Mesin_geprek_model');
	...
	public function index()
	{
		...
		if ($q <> '') {
			...
			$config['first_url'] = base_url() . 'index.php/mesingeprek/index.html?q=' . urlencode($q);
		} else {
			$config['base_url'] = base_url() . 'index.php/mesingeprek/index/';
			$config['first_url'] = base_url() . 'index.php/mesingeprek/index/';
		}

		$config['per_page'] = 10;
		$config['page_query_string'] = FALSE;
		$config['total_rows'] = $this->Mesin_geprek_model->total_rows($q);
		$mesingeprek = $this->Mesin_geprek_model->get_limit_data($config['per_page'], $start, $q);
		$mesingeprek_peruser = $this->Mesin_geprek_model->get_limit_peruser($config['per_page'], $start, $q);
		...
		$data = array(
			'mesingeprek_data' => $mesingeprek,
			'mesingeprek_data_peruser' => $mesingeprek_peruser,
			'q' => $q,
			'pagination' => $this->pagination->create_links(),
			'total_rows' => $config['total_rows'],
			'start' => $start,
		);
		$this->template->load('template', 'mesingeprek/mesin_geprek_list', $data);
	}

	public function read($id)
	{
		$row = $this->Mesin_geprek_model->get_by_id($id);
		if ($row) {
			$data = array(
				'id_mesin_geprek' => $row->id_mesin_geprek,
				'no_wo' => $row->no_wo,
				'id_user' => $row->id_user,
			...
			$this->template->load('template', 'mesingeprek/mesin_geprek_read', $data);
		} else {
			$this->session->set_flashdata('message', 'Record Not Found');
			redirect(site_url('mesingeprek'));
		}
	}
	...
	public function create()
	{
		$data = array(
			'button' => 'Create',
			'action' => site_url('mesingeprek/create_action'),
			'id_mesin_geprek' => set_value('id_mesin_geprek'),
		...
		$this->template->load('template', 'mesingeprek/mesin_geprek_form', $data);
	}


	// TAMBAH DATA MANUAL TANPA AUTOFILL
	public function create_manual()
	{
		$data = array(
			'button' => 'Create',
			'action' => site_url('mesingeprek/create_action_manual'),
			'id_mesin_geprek' => set_value('id_mesin_geprek'),
			'no_wo' => set_value('no_wo'),
			'id_user' => set_value('id_user'),
			'operator2' => set_value('operator2'),
		...
		$this->template->load('template', 'mesingeprek/mesin_geprek_form_manual', $data);
	}
	...
	public function create_action()
	{
		...
		$lembarPack = $this->input->post('id_lembar', TRUE);
		$namaWarna = $this->input->post('id_warna', TRUE);
		...
		$ukuran = getFieldValue('ukuran', 'id_ukuran', 'ukuran', $ukuran);
		$bentuk = getFieldValue('bentuk', 'id_bentuk', 'bentuk', $namaBentuk);
		$lembar = getFieldValue('lembar_pack', 'id_lembar', 'lembar', $lembarPack);
		$warna = getFieldValue('warna', 'id_warna', 'warna', $namaWarna);
		$id_customer = getFieldValue('customer', 'id_customer', 'nama_customer', $namaCustomer);
		if ($this->form_validation->run() == FALSE) {
			...
			$this->Mesin_geprek_model->insert($data);
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'action' => site_url('mesingeprek/update_action'),
				'id_mesin_geprek' => set_value('id_mesin_geprek', $row->id_mesin_geprek),
	...
	public function update_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			$this->update($this->input->post('id_mesin_geprek', TRUE));
		} else {
			$data = array(
				'no_wo' => $this->input->post('no_wo', TRUE),
				'id_user' => $this->session->userdata('id_users'),
			...
			$this->Mesin_geprek_model->update($this->input->post('id_mesin_geprek', TRUE), $data);
			$this->session->set_flashdata('message', 'Update Record Success');
			redirect(site_url('mesingeprek'));
		}
	}

	public function reapet_manual($id)
	{
		$row = $this->Mesin_geprek_model->get_by_id($id);
	...
	public function delete($id)
	{
		...
		if ($row) {
			$this->Mesin_geprek_model->delete($id);
	...
	public function _rules()
	{
		...
		$this->form_validation->set_rules('operator2', 'operator2', 'trim');
		$this->form_validation->set_rules('id_customer', 'id member', 'trim|required');
		...
		$this->form_validation->set_rules('id_mesin_geprek', 'id_mesin_geprek', 'trim');
	...
	public function excel()
	{
		...
		foreach ($this->Mesin_geprek_model->get_all() as $data) {
			$kolombody = 0;
...
/* End of file Mesingeprek.php */
/* Location: ./application/controllers/Mesingeprek.php */
