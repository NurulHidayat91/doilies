...
class Operatormesin2 extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($q <> '') {
			$config['base_url'] = base_url() . 'index.php/operatormesin2/index?q=' . urlencode($q);
			$config['first_url'] = base_url() . 'index.php/operatormesin2/index.html?q=' . urlencode($q);
		} else {
			$config['base_url'] = base_url() . 'index.php/operatormesin2/index/';
			$config['first_url'] = base_url() . 'index.php/operatormesin2/index/';
		}

		$config['per_page'] = 5;
		$config['page_query_string'] = FALSE;
		$config['total_rows'] = $this->Operator_mesin_model->total_rows($q);
		$operatormesin = $this->Operator_mesin_model->get_limit_data_T($config['per_page'], $start, $q);
		$data_peruser = $this->Operator_mesin_model->get_limit_peruser_T($config['per_page'], $start, $q);
		...
		$this->template->load('template', 'operatormesin/operator_mesin_list2', $data);
	}
	...
	public function create()
	{
		...
		$lebar = $this->lebar_bahan_model->get_all();
		$gsm = $this->gsm_model->get_all();
		...
		$data = array(
			'button' => 'Create',
			'row'	 => $row,
			'action' => site_url('operatormesin2/create_action'),
		...
		$this->template->load('template', 'operatormesin/operator_mesin_form2', $data);
	}
	...
	public function create_action()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect(site_url('operatormesin2'));
		}
	}
	...
	public function update($id)
	{
		...
		if ($row) {
			$data = array(
				'button' => 'Update',
				'row' => $row,
				'action' => site_url('operatormesin2/update_action'),
				'id_operator_mesin' => set_value('id_operator_mesin', $row->id_operator_mesin),
				'id_user' => set_value('id_user', $row->id_user),
				'no_form' => set_value('no_form', $row->no_form),
				'operator2' => set_value('operator2', $row->operator2),
	...
	public function autofill()
	{
		...
		$query_lembar    = $this->db->query("select tbl_target_mesin_t.*, speed_mesin, bentuk, nama_pattern from tbl_target_mesin_t
		INNER JOIN speed_mesin  ON tbl_target_mesin_t.id_speed_mesin = speed_mesin.id_speed_mesin		
		INNER JOIN bentuk ON tbl_target_mesin_t.id_bentuk = bentuk.id_bentuk		
		INNER JOIN pattern ON tbl_target_mesin_t.id_pattern = pattern.id_pattern		
		where id_lembar='$lembar' and id_ukuran='$ukuran' and id_grade='$grade' and id_gsm='$gsm' and lebar_roll='$lebar' and id_warna='$warna'")->row_array();
		// var_dump($query_lembar);
		// die;
